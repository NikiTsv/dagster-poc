...
@dg.asset(
    ...
    group_name="analysis",
    deps=[joined_data],
    # recalculate if joined_data is updated
...
@dg.asset(
    compute_kind="duckdb",
    group_name="asynctest",
)
async def async_asset(duckdb: DuckDBResource) -> dg.MaterializeResult:
    with duckdb.get_connection() as conn:
        conn.execute(
            """
            create or replace table async_result as (
                select * from read_csv_auto('data/sales_reps.csv')
            )
            """
        )
        ...
        preview_query = "select * from async_result limit 10"
        preview_df = conn.execute(preview_query).fetchdf()
        row_count = conn.execute("select count(*) from async_result").fetchone()
        count = row_count[0] if row_count else 0
        ...
        return dg.MaterializeResult(
            metadata={
                "row_count": dg.MetadataValue.int(count),
                "preview": dg.MetadataValue.md(preview_df.to_markdown(index=False)),
                "custom_message": dg.MetadataValue.text("Async works"),
            }
        )
